...
Desde sua proposição como extensão do problema clássico das p-medianas \cite{hakimi1964optimum}, o PPMC tem sido estudado tanto por métodos exatos quanto por abordagens heurísticas, motivado por aplicações em planejamento de saúde, logística urbana e alocação de recursos \citep{revelle1996plant, drezner1996facility}. Métodos exatos, incluindo enumeração implícita~\cite{mulvey1984solving}, \textit{branch-and-cut} e geração de colunas \cite{lorena2004column, ceselli2005branch, boccia2008cut}, garantem otimalidade, mas têm custo computacional proibitivo em instâncias de porte médio a grande, o que motivou o desenvolvimento de abordagens heurísticas e metaheurísticas como GRASP~\cite{resende2002grasp}, algoritmos genéticos~\cite{correa2004genetic} e busca
tabu~\cite{osman1994capacitated}.

Diferentemente da aplicação clássica de \textit{Clustering Search} ao 
PPMC, como em \cite{chaves2008clustering}, nas quais o CS atua como mecanismo de agrupamento e intensificação sobre regiões promissoras, em nosso trabalho o CS é usado como critério de acionamento para uma fase de intensificação da busca, através do uso do modelo de programação matemática. Assim, o CS deixa de ser apenas uma estratégia de organização da busca e passa a controlar seletivamente o uso de otimização exata dentro do framework principal.

Na linha de abordagens híbridas, existe o trabalho de \cite{stefanello2015matheuristics} que propõe o \textit{Iterated Reduction Matheuristic Algorithm} (IRMA), baseado na redução do espaço de busca e resolução de subproblemas via programação inteira e \cite{vasconcelos2021algoritmo} aplicam o \textit{General Variable Neighborhood Search} (GVNS) ao PPMC, explorando múltiplas
estruturas de vizinhança sistematicamente. Mais recentemente, \cite{freitas2025ils_pmediana} descrevem uma metaheurística baseada em \textit{Iterated Local Search} voltada especificamente ao PPMC, que constitui o comparativo direto deste trabalho por se tratar da contribuição mais recente da literatura e por operar no mesmo regime de tempo-qualidade da abordagem proposta. A contribuição deste trabalho é a integração de \textit{Clustering Search}, \textit{Adaptive ILS} e intensificação via chamadas de programação inteira mista em um único framework, equilibrando diversificação e intensificação ao longo da busca.
...
\While{$k < \texttt{NumIterMax}$ \textbf{e} tempo $< T_{\max}$}
    ...
    \State $s' \gets \mathrm{VND}(s')$ \Comment{Seção~\ref{subsec:vnd}}
...
Esta seção avalia o desempenho do Hybrid-CS-AILS frente ao ILS de \cite{freitas2025ils_pmediana} (SBPO 2025), comparativo direto da abordagem proposta.  Na Seção \ref{subsec:implement_inst} detalha-se a configuração da implementação, parâmetros adotados e instâncias utilizadas. Em seguida, são apresentadas as análises de qualidade (Seção \ref{sec:results:overview}) e de tempo computacional (Seção \ref{subsec:time}) das soluções obtidas.

\subsection{Ambiente e Protocolo Experimental}
\label{subsec:implement_inst}

Os experimentos foram executados em uma máquina com processador Intel Core i7-11800H (4,60 GHz) com 16 GB de RAM, sob sistema operacional Linux. O solver MILP utilizado no \emph{Partial Optimizer} foi implementado no \textsc{IBM CPLEX}~\textit{22.1.1}. O algoritmo foi implementado em C++17 e compilado com \texttt{-O3}. O executável e os resultados detalhados estão disponíveis no repositório: \url{https://github.com/anonymous/Hybrid_CS_AILS} .
...
distintos de $p$ cada. Cada instância foi executada $N=10$ vezes com sementes aleatórias independentes. Os parâmetros $\alpha$ (GRASP), $\gamma$ e $v_{\max}$ (\textit{Clustering Search}) e $\beta_{R2}$ (\textit{Partial Optimizer}) foram calibrados automaticamente com o pacote \texttt{irace}~\cite{LopezIbanez2016irace}, que aplica um procedimento de \textit{racing} iterado sobre configurações candidatas, eliminando as estatisticamente inferiores em um conjunto de treinamento. A calibração empregou cinco instâncias de treino de famílias de instâncias distintas, três \textit{seeds} por instância e orçamento de 300 experimentos; a configuração elite obtida foi $\alpha = 0{,}6667$, $\gamma = 18$, $v_{\max} = 8$ e $\beta_{R2} = 16$. Após a calibração, todos os experimentos finais foram executados com $\texttt{NumIterMax}=60$. 


Os valores de custo e de tempo médio de execução do ILS utilizados nas
comparações são os reportados pelos autores na publicação
original~\cite{freitas2025ils_pmediana}. Para cada instância, denotamos por $Z^{\star}_{\mathrm{ILS}}$
o melhor custo reportado pelo ILS entre suas execuções. A qualidade de
uma solução $Z$ obtida pelo Hybrid-CS-AILS é medida pelo \emph{gap}
percentual relativo a essa referência:
\begin{equation}
  \mathrm{gap}(Z) = \frac{Z - Z^{\star}_{\mathrm{ILS}}}{Z^{\star}_{\mathrm{ILS}}} \cdot 100\,\%,
  \label{eq:gap}
\end{equation}
em que valores negativos indicam que o Hybrid-CS-AILS supera o ILS na
métrica considerada.
...
obtido entre as $N=20$ execuções e por $Z_{\mathrm{mean}}$ o custo
médio, e reportamos os \emph{gaps} percentuais correspondentes em
relação ao ILS (Equação~\ref{eq:gap}). A Tabela~\ref{tab:family} consolida o desempenho do Hybrid-CS-AILS por
família de instâncias. O coeficiente de variação médio (CV\%), que mede a dispersão
relativa dos custos entre as 10 execuções, permanece abaixo de
$0{,}45\%$ em todas as sete famílias de instâncias, indicando estabilidade do
método. Em termos de $Z_{\mathrm{mean}}$, o Hybrid-CS-AILS supera o ILS
em seis das sete famílias, com $\overline{\mathrm{gap}}_{\mathrm{mean}}$
negativo em todas elas exceto \texttt{p3038}, e o ganho global é
de $-0{,}788\%$, com 22 vitórias contra 13 nas 35 instâncias avaliadas.

\begin{table}[!htb]
\centering
\caption{Resultados agregados por família de instâncias, com \emph{gaps} percentuais calculados em relação ao ILS de \cite{freitas2025ils_pmediana}
(Equação~\ref{eq:gap})}
\label{tab:family}
\small
...
Família & \#inst. & $\overline{\mathrm{gap}}_{\mathrm{best}}$ (\%)
        ...
        & CV (\%) & ganhos/perdas & Tempo (h) \\
\hline
\texttt{lin318}  & 5 & $-0{,}129$ & $-0{,}210$ & $0{,}215$ & 3/2 &  3{,}15 \\
\texttt{ali535}  & 5 & $-0{,}339$ & $-2{,}788$ & $0{,}344$ & 5/0 &  4{,}03 \\
\texttt{u724}    & 5 & $-0{,}405$ & $-0{,}645$ & $0{,}357$ & 4/1 &  2{,}62 \\
\texttt{rl1304}  & 5 & $+0{,}110$ & $-0{,}278$ & $0{,}404$ & 2/3 &  6{,}73 \\
\texttt{pr2392}  & 5 & $-0{,}087$ & $-1{,}346$ & $0{,}320$ & 3/2 & 16{,}47 \\
\texttt{fnl4461} & 5 & $+0{,}263$ & $-0{,}630$ & $0{,}422$ & 4/1 & 32{,}11 \\
\texttt{p3038}   & 5 & $+0{,}783$ & $+0{,}383$ & $0{,}376$ & 1/4 & 49{,}30 \\
\hline
\textbf{Global}  & 35 & $+0{,}028$ & $-0{,}788$ & $0{,}348$ & 22/13 & --- \\
\hline
\end{tabular}
\end{table}

A vantagem observada em $Z_{\mathrm{mean}}$ é confirmada pelo teste
pareado de Wilcoxon \emph{signed-rank}, aplicado sobre a diferença dos
custos médios com hipótese alternativa unilateral favorecendo o método
proposto: rejeita-se a hipótese nula ao nível $\alpha = 0{,}05$ ($p =
0{,}011$, 22 vitórias contra 13 em 35 instâncias). A análise da
distribuição dos \emph{gaps} ao longo das 35 instâncias evidencia que,
em $Z_{\mathrm{best}}$, os dois métodos são equivalentes em qualidade
(mediana próxima de zero), enquanto, em $Z_{\mathrm{mean}}$, o
Hybrid-CS-AILS apresenta variabilidade menor entre execuções, traduzindo-se em soluções médias mais consistentes que as do ILS.
...
e tempo de execução são avaliados juntos. Na Figura~\ref{fig:scatter}
apresentamos, em escala logarítmica no eixo horizontal, o tempo médio
versus o custo médio (normalizado pelo melhor valor entre os
dois métodos) para cada uma das 35 instâncias do Hybrid-CS-AILS e do
ILS de \cite{freitas2025ils_pmediana}. O Hybrid-CS-AILS domina
o ILS em praticamente toda a extensão do gráfico: para cada ponto do
ILS existe um ponto do Hybrid-CS-AILS melhor em qualidade e tempo. 

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.78\linewidth]{figuras/time_quality_scatter.pdf}
  \caption{Fronteira tempo $\times$ qualidade entre Hybrid-CS-AILS e
  ILS \cite{freitas2025ils_pmediana} sobre as 35 instâncias. Cada
  ponto representa o tempo médio por execução de uma instância;
  para o ILS, os valores são os reportados na publicação original.
  O eixo $y$ é o \emph{gap} percentual do custo médio em relação ao
  melhor entre os dois métodos em cada instância.}
  \label{fig:scatter}
\end{figure}



% O ganho em tempo foi quantificado pela média geométrica das razões
% $T_{\mathrm{ILS}}/T_{\mathrm{Hybrid}}$ por instância
% \citep{fleming1986not}. Sem normalização de hardware, o Hybrid-CS-AILS é
% $6{,}7\times$ mais rápido que o ILS; aplicando uma correção conservadora
% baseada no \emph{Single Thread Rating} dos processadores reportados
% (Intel Core i5-7200U para o ILS e Intel Core i7-11800H neste trabalho),
% esse ganho permanece em aproximadamente $3{,}8\times$. A Tabela~\ref{tab:speedup}
% consolida os resultados por família, com maiores ganhos em
% \texttt{ali535} e \texttt{u724}, enquanto \texttt{p3038} apresenta ganho
% mais modesto devido ao maior esforço do \textit{Partial Optimizer} quando
% $p/n \geq 0{,}3$.



O \emph{ganho} em tempo foi quantificado pela razão entre o tempo médio
reportado para o ILS e o tempo médio obtido pelo Hybrid-CS-AILS em cada
família de instâncias, conforme consolidado na Tabela~\ref{tab:speedup}.
De modo geral, o Hybrid-CS-AILS é $3{,}3\times$ mais rápido que o ILS,
com ganhos expressivos em \texttt{rl1304} ($8{,}4\times$),
\texttt{ali535} ($7{,}9\times$) e \texttt{u724} ($7{,}9\times$).
A única família de instâncias em que o ganho é menor é na família de instâncias \texttt{p3038},
em que a razão $p/n \geq 0{,}3$ leva o \textit{Partial Optimizer} a
consumir mais tempo computacional.
...
A combinação de qualidade superior e tempo de execução de uma ordem de grandeza menor deixa evidente um espaço de melhoria ainda não explorado pela abordagem do ILS de \cite{freitas2025ils_pmediana} (SBPO 2025) reportadas para o PPMC.


\section{Conclusão}
\label{secCon}

Portanto, neste trabalho foi proposta uma matheurística híbrida denominada \textit{Hybrid-CS-AILS} para o Problema das p-Medianas Capacitado (PPMC). A abordagem integra mecanismos heurísticos e exatos por meio da combinação entre uma heurística construtiva baseada em GRASP, busca local VND, um framework adaptativo de \textit{Iterated Local Search} e um mecanismo de intensificação seletiva guiado por \textit{Clustering Search}. Diferentemente de abordagens híbridas tradicionais, o método proposto utiliza o \textit{Clustering Search} para auxiliar na decisão de quando aplicar o modelo do PPMC via programação inteira mista, concentrando esforço computacional em regiões promissoras do espaço de busca.

Os experimentos computacionais realizados em 35 instâncias benchmark clássicas do PPMC mostraram que o Hybrid-CS-AILS supera o ILS de \cite{freitas2025ils_pmediana}, comparativo direto e referência mais recente da literatura, em termos de custo médio com significância estatística ($p = 0{,}011$ no teste pareado de Wilcoxon) e produz soluções, em média, $6{,}7$ vezes mais rapidamente. Os resultados evidenciaram, ainda, elevada estabilidade entre execuções independentes em todas as sete famílias avaliadas.
...
% Os autores agradecem ao Programa de Engenharia de Sistemas e Computação (PESC/COPPE/UFRJ) pelo suporte institucional, e às agências de fomento FAPERJ: E-26/204.211/2025, CNPq: 304043/2025-7, UFRJ ALV Grant: 23079.227622/2023-70 pelo financiamento parcial desta pesquisa.
