...
inteira, e \cite{vasconcelos2021algoritmo} aplicam o \textit{General
Variable Neighborhood Search} (GVNS) ao PPMC, combinando múltiplas
estruturas de vizinhança em um esquema sistemático. Mais recentemente,
\cite{freitas2025ils_pmediana} descrevem uma metaheurística baseada em
\textit{Iterated Local Search} voltada especificamente ao PPMC, que
constitui o comparativo direto deste trabalho por se tratar da
contribuição mais recente da literatura e por operar no mesmo regime
de tempo-qualidade visado pela abordagem proposta. A contribuição
deste trabalho é a integração de \textit{Clustering Search},
\textit{Adaptive ILS} e intensificação via chamadas de programação
inteira mista em um único framework, equilibrando diversificação e
intensificação ao longo da busca.
...
Para cada instância, denotamos por $Z^{\star}_{\mathrm{ILS}}$ o melhor
custo reportado pelo ILS \cite{freitas2025ils_pmediana} entre suas
execuções. A qualidade de uma solução $Z$ obtida pelo Hybrid-CS-AILS é
medida pelo \emph{gap} percentual relativo a essa referência:
\begin{equation}
...
\begin{table}[!htb]
\centering
\caption{Resultados agregados por família, com \emph{gaps} percentuais
...
médio dos custos entre as 10 execuções; \emph{wins/losses} compara o
...
Hybrid-CS-AILS apresenta variabilidade entre execuções
substancialmente menor, traduzindo-se em soluções médias mais
consistentes que as do ILS.
...
ILS de \cite{freitas2025ils_pmediana}. O Hybrid-CS-AILS \emph{Pareto}-domina
o ILS em praticamente toda a extensão do gráfico: para cada ponto do
ILS existe um ponto do Hybrid-CS-AILS com tempo menor \emph{e}
qualidade melhor.
...
\begin{figure}[!htb]
  ...
  ILS \cite{freitas2025ils_pmediana} sobre as 35 instâncias.}
  \label{fig:scatter}
\end{figure}

A magnitude do ganho temporal é quantificada pela média geométrica
das razões $T_{\mathrm{ILS}} / T_{\mathrm{Hybrid}}$ por instância,
agregada na Tabela~\ref{tab:speedup}. Globalmente, o Hybrid-CS-AILS é
$6{,}7\times$ mais rápido que o ILS, com ganhos particularmente
expressivos em \texttt{ali535} ($19{,}0\times$) e \texttt{u724}
($15{,}4\times$). A única família em que o ganho é modesto é
\texttt{p3038}, em que a razão $p/n \geq 0{,}3$ leva o \textit{Partial
Optimizer} a um regime degradado.

\input{tabelas/T_speedup.tex}

A combinação dos dois resultados --- qualidade média superior e tempo
de execução de uma ordem de grandeza menor --- caracteriza uma região
da fronteira tempo $\times$ qualidade ainda não explorada pelas
abordagens previamente reportadas para o PPMC.
